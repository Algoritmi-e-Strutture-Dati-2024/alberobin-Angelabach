...
public:
    ...
    int figlioSinistro(int indice) const {
        int posizione = 2 * indice + 1;
        if (indice >= 0 && posizione < 100 && occupato[posizione]) {
            return posizione;
        }
        return -1;
    }
    ...
    void insFiglioSinistro(int indice, const T& valore) {
        int posizione = 2 * indice + 1;
        if (indice >= 0 && posizione < 100 && !occupato[posizione]) {
            nodi[posizione] = valore;
            occupato[posizione] = true;
        }
    }
    ...
    void stampa() const {
        for (int i = 0; i < 100; ++i) {
            if (occupato[i]) {
                ...
                std::cout << " Posizione " << i << " Valore: " << valore << " Figliosinistro: ";
